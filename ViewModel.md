...
Первые костыли мы всегда наблюдаем при попытках задать модели значения по умолчанию - для этого у нас есть свойство - ассоциативный массив $attributes. Можно частенько наблюдать такой код:
...
Да, в этом куске кода мы вроде как безусловно ослабили связь значений по умолчанию, но теперь вы всегда должны будете помнить, что у вас в моделе родителя в $defaults может что-то уже сидеть итд итп. 

Что бы мы ещё могли ракритивать в Eloquent-моделях? Да опять всё те же аттрибуты - они инкапсулированны и будут пополняться/обновляться через __set() всякий раз, когда вы присваиваете свойству модели какое-либо значение. То есть нас абсолютно не устраивает инкапсуляция аттрибутов - она требует излишный контроль какими-то негибкими средствами: будь то $fillable или $guarded, будь то Observer, который перед сохранением модели будет проверять что сохранить, а что нет, будь то что угодно, что не попадает под значения "удобное" или "несвязанное". К тому же инкапсуляция аттрибутов выводит многих из себя, лишая людей тайпхинтинга. Если кто-то скажет "поставь себе ide-helper и сгенерируй описание модели через artisan", то я буду в растерянности - пологаться на docHere модели, которую бог-весть когда описали(ведь возможно описали всего один раз) - не самый лучший метод, да и при надобности с моделью всё равно не сможешь "порефлексировать".

Так что же модели в Laravel? Из всего вышесказанного я могу сделать вывод, что Eloquent отличная вещь для моего непринуждённого общения с бд, но не для ведения сущностей в проекте - высока цена (в рефлексии свыше 120 искоробочных методов и не факт что они понадобяться), неудобство для контекстного положения ну и конечно же жёсткая привязанность моделей данных к транспортному слою, хоть модель сохраняющая себя хороша, но только как задумка в маленьких проектах.

Сделав пару выводов я пришёл к тому, что слой моделей надо реализовавать отдельно от Eloquent, сделав так называемый ViewModel подход и написав "умный контроллер" и хочу поделиться набросками с вами.

Я знаю, что подход работы с данными через репозитории и их инъекцию неоднократно затрагивался в разных статьях по laravel, но хотелось бы рассмотреть разделение ответственности между транспортными слоями, которая подойдёт для большинства ActiveRecord решений. Под разделением транспортных слоёв я имею ввиду чёткое разграничение контейнера запросов данных от контейнера записи, хоть у этих слоёв будет 2 общих метода: один, возвращающий базовую модель, второй возвращающий модель найденую по первичному ключу. Тогда с этого и можно начать проектирование - напишем интерфейс к оснаванию транспортного слоя IDataTransport.

    namespace Application\System\DAL\Interfaces;
...
Наши интерфейсы для сервиса и репозитория мы будем наследовать от IDataTransport. В репозитории нам по сути нужно всего 4 метода - создание модели из данных, сохранение созданной модели, обновление моделей из данных по ключу и простое обновление переданной модели. Вот так будет выглядеть IDataRepotitory:
...
Сразу напишем абстрактную реализацию, пригодную для Eloquent:
    ...
    abstract class AEloquentRepository implements IDataRepository
    {
        ...
        public funtion createModel($model)
        {
            return $model->save();
        }
...
В основном интерфейсе сервиса нам тоже много не понадобиться, но всё же основные методы мы опишем. 

    namespace Application\System\DAL\Interfaces;
    
    interface IDataService extends IDataTransport
    {
        public function all();
