...
<ul>
    <li><code>$lower</code> — приведёт полученную по маске строку в lowercase. По умолчанию ```true```;</li>
    <li><code>$underscore</code> — в полученной по маске строке не будет заменять «_» на «.». По умолчанию ```false```;</li
