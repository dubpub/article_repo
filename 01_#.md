...
<ul>
    <li>```$lower``` — приведёт полученную по маске строку в lowercase. По умолчанию ```true```;</li>
    <li>```$underscore``` — в полученной по маске строке не будет заменять «_» на «.». По умолчанию ```false```;</li
    <li>```$clearFixes``` — уберёт из полученной по маске строки ```$prefix``` и ```$postfix```.</li>
</ul>
