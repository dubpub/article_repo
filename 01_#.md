...
Перед тем как приступать к приложению было бы неплохо автоматизировать работу с самим laravel. В этой части хотелось бы решить проблему безответственного захламления как своего приложения, так и глобального пространства имён такими файлами как «mymailevents.php», «di.php», «observers.php», и вы те поверите, но в этот список так же входит «filters.php». В вышеперечисленном списке скопом идут неупорядоченные объявления событий, инъекций зависимостей, навешивания Observer’ов объектов на модели.
...
Как бы вы не хотели -  всё содержимое этих файлов наверняка будет вызываться во время каждого обращения к вашему приложению. Например: зачем мне регистрировать события рассылки писем, навешивать Observer на модель News, если пользователь просто попал на страницу авторизации?  Этот вопрос плавно подведёт нас ко второй части — организации областей приложения, ведь многие поключают эти файлы прямо из «app/start/global.php», а иногда, что самое ужесное — прямо из «app/routes.php» или «app/filters.php».
А что самое главное, давно пора мыслить объектно — всему своя ответвственность.

Для начала решим первый вопрос — вопрос с хранением и подгрузкой по требованию наших расширений для стандартных помощников L4 и пройдёмся по списку того, что нам будет нужно сделать максимально комфортным и тестируемым:
Загрузка фильтров (\Route::filter);
Регистрация зависимостей IoC контейнера (\App::bind);
Регистрация событий (\Event::listen);
Загрузка макросов формы (\Form::macro);
Загрузка собственных расширений валидатора (\Validator::extend);
Расширение Blade компилятора (\Blade::extend);
...
Направляемся в папку ./Appication и создаём в ней папку Abstracts, в которой в свою очередь создаём папку Helpers. Тут же создадим базовый абстрактный класс WakeUpWrapper:
	<?php
		//# ./app/Application/Abstracts/Helpers/WakeUpWrapper.php
  		...
  		abstract class WakeUpWrapper
   		 {
       			...
       			public $lower = true;
        			...
        			public $clearify = true;
        			public $prefix = "";
        			public $postfix = "";

        			abstract public function awaken($object, $method, $name);

       			 public static function boot()
        			{		
            			$instance = App::make(get_called_class());
            			...
            			$getExp = function($_fix)
            			{
                				if (is_array($_fix))  {
                    				return "(".implode("|", $_fix).")";
                				} elseif(is_string($_fix) && mb_strlen($_fix)) {
                    				return "({$_fix})";
                				}
            			...
            			$exp = "/".$getExp($instance->prefix)."([A-Za-z0-9\\_])".$getExp($instance->postfix)."/";
				...
				$filter = function($method) use($reflect, $exp)
            			...
            			$items = array_filter($instance->wrapMethods ? $reflect->getMethods() : $reflect->getProperties(), $filter);

            			foreach($items as $reflected)
            			{
                				$filterName = $reflected->getName();

                				if ($instance->clearify) {
                    				$filterName = str_replace($instance->postfix, "", $reflected->getName());
                    				$filterName = str_replace($instance->prefix, "", $filterName);
                				}

                				if (!$instance->underscore) {
               				...
               				if ($instance->lower) {
                				...
                				$instance->awaken($instance, $reflected, $filterName);
            			}
        			}
    		}
